...
static void* get_cmts(void* arg_poolname) {
  ...
  for (;;) {
    ...
    struct nlmsghdr* nlh = (struct nlmsghdr*) malloc(NLMSG_SPACE(max_buffer_size()));
    ...
    recv_cmt_msg_t* recv_msg = deserialize_recv_cmt(reinterpret_cast<char*>(NLMSG_DATA(nlh)));
