...
static char *serialize_recv_cmt(char *recv_msg, const char *poolname,
                                uint64_t blk_id, const char *tail_commitment) {

  memcpy(recv_msg, &(blk_id), sizeof(blk_id));
  memcpy(recv_msg + sizeof(blk_id), poolname, ZFS_MAX_DATASET_NAME_LEN);
  memcpy(recv_msg + sizeof(blk_id) + ZFS_MAX_DATASET_NAME_LEN, tail_commitment,
         COMMITMENT_SIZE);
  return recv_msg;
}

static int get_size_of_recv_cmt(void) { return sizeof(recv_cmt_msg_t); }
...
static void notify_cmts(struct sk_buff *skb) {
  ...
  printk(KERN_INFO
         "notify_cmts (current pid=%d): received notification from pid=%d\
    about blk_id=%lld \
...
static int __init netlink_module_init(void) {
  ...
  nl_sock_get_cmts =
      netlink_kernel_create(&init_net, GET_CMTS_SOCK, &cfg_get_cmts);
  if (!nl_sock_get_cmts) {
  ...
  nl_sock_notify =
      netlink_kernel_create(&init_net, NOTIFY_CMTS_SOCK, &cfg_notify_cmts);
  if (!nl_sock_notify) {
    printk(KERN_ALERT
           "netlink_module: error creating NOTIFY_CMTS_SOCK socket.\n");
    return -10;
  }
#endif
