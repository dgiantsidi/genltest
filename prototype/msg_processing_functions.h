...
MAYBE_UNUSED static char *serialize_notify_cmt_into_char(const char *poolname,
                                                         uint64_t blk_id) {
  char *serialized_msg = (char *)malloc(sizeof(notify_cmt_msg_t));
...
MAYBE_UNUSED static char *serialize_get_cmt_into_char(const char *poolname) {
  char *serialized_msg = (char *)malloc(sizeof(get_cmt_msg_t));
  memcpy(serialized_msg, poolname, ZFS_MAX_DATASET_NAME_LEN);
  return serialized_msg;
}

MAYBE_UNUSED static recv_cmt_msg_t *deserialize_recv_cmt(char *msg) {
